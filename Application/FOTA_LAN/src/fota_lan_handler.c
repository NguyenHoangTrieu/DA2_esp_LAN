 ...
 * Enhanced with eppp_link for PPP internet connection
 */
...
{
    ESP_LOGI(TAG, "Connecting to PPP server for internet access...");
...
void fota_lan_handler_advanced_ota_task(void *pvParameter) 
{
    ...
    //  Use PPP interface instead of bind interface name
    struct ifreq ifr;
...
void fota_lan_handler_task_start(void) 
{
    ...
    // Connect to PPP server for internet access
    ...
    xTaskCreate(&fota_lan_handler_advanced_ota_task, "fota_lan_ota_task", 32 * 1024, NULL, 5, NULL);
}
