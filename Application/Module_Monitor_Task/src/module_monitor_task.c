...
#include "module_monitor_task_internal.h"
#include "esp_log.h"
#include "nvs_flash.h"
#include "nvs.h"
...
static esp_err_t module_parse_json_config(uint8_t stack_id, const char *json_str, uint16_t json_len);
static esp_err_t module_detect_type_from_json(const char *json_str, module_type_t *module_type);
static esp_err_t module_start_handler_task(uint8_t stack_id, module_type_t module_type);
...
esp_err_t module_monitor_task_start(void) {
    ...
    for (int i = 0; i < MODULE_MONITOR_MAX_STACKS; i++) {
        ...
        if (module_monitor_load_config_from_nvs(i, &json_str, &json_len) == ESP_OK) {
    ...
    BaseType_t task_ret = xTaskCreate(
        module_monitor_task_impl,
        "module_monitor",
        MODULE_MONITOR_TASK_STACK_SIZE,
        NULL,
...
esp_err_t module_monitor_load_config(uint8_t stack_id, const char *json_str, uint16_t json_len) {
    if (!g_monitor_state.initialized || !json_str || json_len == 0 || stack_id > 1) {
    ...
    if (xQueueSend(g_monitor_state.config_queue, &msg, pdMS_TO_TICKS(1000)) != pdTRUE) {
...
handler_status_t module_monitor_get_handler_status(uint8_t stack_id) {
    ...
    if (xSemaphoreTake(g_monitor_state.mutex, pdMS_TO_TICKS(100)) == pdTRUE) {
        handler_status_t status = g_monitor_state.module_info[stack_id].handler_status;
        xSemaphoreGive(g_monitor_state.mutex);
        return status;
    }
...
esp_err_t module_monitor_save_config_to_nvs(uint8_t stack_id, const char *json_str, uint16_t json_len) {
    ...
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "Config saved to NVS for Stack %d (%u bytes)", stack_id, json_len);
    } else {
...
esp_err_t module_monitor_load_config_from_nvs(uint8_t stack_id, char **json_str, uint16_t *json_len) {
    ...
    *json_len = (uint16_t)required_size;
    ...
    ESP_LOGI(TAG, "Config loaded from NVS for Stack %d (%u bytes)", stack_id, *json_len);
    return ESP_OK;
}
...
static esp_err_t module_stop_handler_task(uint8_t stack_id) {
    ...
    ESP_LOGI(TAG, "Stopping handler for Stack %d (type=%d)", stack_id, info->module_type);
...
static void module_monitor_task_impl(void *pvParameters) {
    ...
    while (g_monitor_state.initialized) {
        ...
        if (xQueueReceive(g_monitor_state.config_queue, &msg, pdMS_TO_TICKS(5000)) == pdTRUE) {
            ESP_LOGI(TAG, "Received config for Stack %d", msg.stack_id);

            // Parse and apply config
            esp_err_t ret = module_parse_json_config(msg.stack_id, msg.json_str, msg.json_len);
            if (ret == ESP_OK) {
                // Save to NVS for persistence
                module_monitor_save_config_to_nvs(msg.stack_id, msg.json_str, msg.json_len);
