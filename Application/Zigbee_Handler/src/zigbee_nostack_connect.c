/**
 * @file zigbee_nostack_connect.c
 * @brief Zigbee No-Stack Connect Task Implementation
 *
 * Glues Zigbee handler <-> WAN MCU handler:
 * - Uplink: zigbee_nostack_handler RX callback -> build packet ->
 * mcu_wan_enqueue_uplink()
 * - Downlink: WAN dispatch -> zigbee_nostack_connect_enqueue_downlink() ->
 * zigbee_nostack_handler_send()
 */

#include "zigbee_nostack_connect.h"
#include "esp_log.h"
#include "esp_timer.h"
#include "frame_types.h"
#include "freertos/FreeRTOS.h"
#include "freertos/queue.h"
#include "freertos/task.h"
#include "mcu_wan_handler.h"
#include <stdlib.h>
#include <string.h>

static const char *TAG = "ZIGBEE_NOSTACK_CONNECT";

/* ===== Configuration ===== */
#define ZIGBEE_CONNECT_TASK_STACK_SIZE 4096
#define ZIGBEE_CONNECT_TASK_PRIORITY 4
#define ZIGBEE_POLL_DELAY_MS 10
#define ZIGBEE_STATS_LOG_INTERVAL_MS 30000
#define ZIGBEE_DOWNLINK_QUEUE_SIZE 20

/* ===== External CC2530 Handle ===== */
/* Must be initialized in zigbee_cc_comm module */
extern zigbee_cc_comm_handle_t g_zigbee_cc_handle;

/* ===== Global Context & State ===== */
zigbee_nostack_handler_ctx_t g_zigbee_nostack_ctx;

typedef struct {
  uint16_t sensor_addr;
  uint16_t data_length;
  uint8_t *data_payload;
} zigbee_downlink_packet_t;

static TaskHandle_t g_zigbee_task = NULL;
static QueueHandle_t g_downlink_queue = NULL;
static bool g_zigbee_running = false;
static zigbee_nostack_connect_stats_t g_stats = {0};

/* ===== Forward Declarations ===== */
static void zigbee_nostack_connect_task(void *pvParameters);
static void zigbee_nostack_connect_rx_cb(const zigbee_nostack_frame_t *frame);
static void zigbee_nostack_connect_log_stats(void);

/* ===== Public API ===== */

esp_err_t zigbee_nostack_connect_start(void) {
  if (g_zigbee_cc_handle == NULL) {
    ESP_LOGE(TAG, "CC2530 handle is NULL! Init failed");
    return ESP_FAIL;
  }

  if (g_zigbee_running) {
    ESP_LOGW(TAG, "Zigbee no-stack connect already running");
    return ESP_OK;
  }

  ESP_LOGI(TAG, "Starting Zigbee no-stack connect");

  /* Initialize handler context */
  zigbee_nostack_handler_init(&g_zigbee_nostack_ctx, g_zigbee_cc_handle);

  /* Register RX callback */
  zigbee_nostack_handler_register_rx_callback(&g_zigbee_nostack_ctx,
                                              zigbee_nostack_connect_rx_cb);

  /* Create downlink queue */
  g_downlink_queue = xQueueCreate(ZIGBEE_DOWNLINK_QUEUE_SIZE,
                                  sizeof(zigbee_downlink_packet_t));
  if (g_downlink_queue == NULL) {
    ESP_LOGE(TAG, "Failed to create downlink queue");
    return ESP_FAIL;
  }

  memset(&g_stats, 0, sizeof(g_stats));

  /* Create processing task */
  BaseType_t ret = xTaskCreate(zigbee_nostack_connect_task, "zigbee_nostack",
                               ZIGBEE_CONNECT_TASK_STACK_SIZE, NULL,
                               ZIGBEE_CONNECT_TASK_PRIORITY, &g_zigbee_task);
  if (ret != pdPASS) {
    ESP_LOGE(TAG, "Failed to create task");
    vQueueDelete(g_downlink_queue);
    g_downlink_queue = NULL;
    return ESP_FAIL;
  }

  g_zigbee_running = true;
  ESP_LOGI(TAG, "Zigbee no-stack connect started");
  return ESP_OK;
}

esp_err_t zigbee_nostack_connect_stop(void) {
  if (!g_zigbee_running) {
    return ESP_OK;
  }

  ESP_LOGI(TAG, "Stopping Zigbee no-stack connect");
  g_zigbee_running = false;

  vTaskDelay(pdMS_TO_TICKS(100));

  if (g_zigbee_task != NULL) {
    vTaskDelete(g_zigbee_task);
    g_zigbee_task = NULL;
  }

  /* Clean up downlink queue */
  if (g_downlink_queue != NULL) {
    zigbee_downlink_packet_t pkt;
    while (xQueueReceive(g_downlink_queue, &pkt, 0) == pdTRUE) {
      if (pkt.data_payload) {
        free(pkt.data_payload);
      }
    }
    vQueueDelete(g_downlink_queue);
    g_downlink_queue = NULL;
  }

  ESP_LOGI(TAG, "Zigbee no-stack connect stopped");
  return ESP_OK;
}

bool zigbee_nostack_connect_enqueue_downlink(uint8_t *data, uint16_t len) {
  if (g_downlink_queue == NULL || data == NULL || len < 4) {
    ESP_LOGE(TAG, "Invalid downlink parameters");
    return false;
  }

  /* Parse: [sensor_addr(2)][length(2)][data...] */
  uint16_t sensor_addr = ((uint16_t)data[0] << 8) | (uint16_t)data[1];
  uint16_t payload_len = ((uint16_t)data[2] << 8) | (uint16_t)data[3];

  if (payload_len == 0) {
    ESP_LOGW(TAG, "Downlink payload length is zero");
    return false;
  }

  if (4 + payload_len > len) {
    ESP_LOGW(TAG, "Downlink buffer too short");
    payload_len = (len > 4) ? (len - 4) : 0;
  }

  if (payload_len == 0 || payload_len > ZIGBEE_NOSTACK_MAX_PAYLOAD) {
    ESP_LOGW(TAG, "Invalid payload length: %u", payload_len);
    return false;
  }

  zigbee_downlink_packet_t packet;
  packet.sensor_addr = sensor_addr;
  packet.data_length = payload_len;
  packet.data_payload = (uint8_t *)malloc(payload_len);

  if (packet.data_payload == NULL) {
    ESP_LOGE(TAG, "Failed to allocate downlink memory");
    return false;
  }

  memcpy(packet.data_payload, data + 4, payload_len);

  if (xQueueSend(g_downlink_queue, &packet, pdMS_TO_TICKS(100)) != pdTRUE) {
    free(packet.data_payload);
    g_stats.downlink_dropped++;
    ESP_LOGW(TAG, "Downlink queue full");
    return false;
  }

  g_stats.downlink_enqueued++;
  ESP_LOGD(TAG, "Downlink enqueued: addr=0x%04X, len=%u", sensor_addr,
           payload_len);
  return true;
}

void zigbee_nostack_connect_get_stat(zigbee_nostack_connect_stats_t *out) {
  if (!out) {
    return;
  }

  zigbee_nostack_stats_t core_stats;
  zigbee_nostack_handler_get_stats(&g_zigbee_nostack_ctx, &core_stats);

  out->tx_ok = core_stats.tx_ok;
  out->rx_ok = core_stats.rx_ok;
  out->rx_error = core_stats.rx_error;
  out->uplink_forwarded = g_stats.uplink_forwarded;
  out->uplink_queue_full = g_stats.uplink_queue_full;
  out->downlink_enqueued = g_stats.downlink_enqueued;
  out->downlink_dropped = g_stats.downlink_dropped;
}

/* ===== Internal Task ===== */

static void zigbee_nostack_connect_task(void *pvParameters) {
  ESP_LOGI(TAG, "Zigbee no-stack connect task started");

  TickType_t last_stats_log = xTaskGetTickCount();

  while (g_zigbee_running) {
    /* ===== RX Polling ===== */
    if (g_zigbee_cc_handle != NULL) {
      uint8_t rx_buf[256];

      while (zigbee_cc_comm_available(g_zigbee_cc_handle) > 0) {
        size_t rx_len = 0;
        esp_err_t rx_status = zigbee_cc_comm_read(g_zigbee_cc_handle, rx_buf,
                                                  sizeof(rx_buf), &rx_len, 0);
        if (rx_status != ESP_OK || rx_len == 0) {
          break;
        }

        /* Pass to handler for processing */
        uint8_t frame_len = (rx_len > 0xFF) ? 0xFF : (uint8_t)rx_len;
        zigbee_nostack_handler_handle_rx(&g_zigbee_nostack_ctx, rx_buf,
                                         frame_len);
      }
    }

    /* ===== Process Downlink Queue ===== */
    zigbee_downlink_packet_t pkt;
    if (xQueueReceive(g_downlink_queue, &pkt, 0) == pdTRUE) {
      uint16_t send_len = pkt.data_length;
      if (send_len > ZIGBEE_NOSTACK_MAX_PAYLOAD) {
        send_len = ZIGBEE_NOSTACK_MAX_PAYLOAD;
      }

      bool sent = false;
      if (send_len > 0) {
        sent =
            zigbee_nostack_handler_send(&g_zigbee_nostack_ctx, pkt.sensor_addr,
                                        pkt.data_payload, (uint8_t)send_len);
      }

      if (!sent) {
        g_stats.downlink_dropped++;
        ESP_LOGW(TAG, "Failed to send downlink (addr=0x%04X, len=%u)",
                 pkt.sensor_addr, send_len);
      } else {
        ESP_LOGD(TAG, "Downlink sent (addr=0x%04X, len=%u)", pkt.sensor_addr,
                 send_len);
      }

      free(pkt.data_payload);
    }

    /* ===== Periodic Statistics ===== */
    TickType_t now_tick = xTaskGetTickCount();
    if ((now_tick - last_stats_log) >=
        pdMS_TO_TICKS(ZIGBEE_STATS_LOG_INTERVAL_MS)) {
      zigbee_nostack_connect_log_stats();
      last_stats_log = now_tick;
    }

    vTaskDelay(pdMS_TO_TICKS(ZIGBEE_POLL_DELAY_MS));
  }

  ESP_LOGI(TAG, "Zigbee no-stack connect task exiting");
  vTaskDelete(NULL);
}

/* ===== RX Callback (Uplink to WAN) ===== */

static void zigbee_nostack_connect_rx_cb(const zigbee_nostack_frame_t *frame) {
  if (!frame || frame->len == 0) {
    return;
  }

  uint16_t sensor_addr = frame->src_id;
  uint16_t payload_len = frame->len;

  if (payload_len > ZIGBEE_NOSTACK_MAX_PAYLOAD) {
    payload_len = ZIGBEE_NOSTACK_MAX_PAYLOAD;
  }

  /* Build uplink packet: [sensor_addr(2)][length(2)][data(length)] */
  uint8_t uplink_buf[4 + ZIGBEE_NOSTACK_MAX_PAYLOAD];
  uplink_buf[0] = (uint8_t)(sensor_addr >> 8);
  uplink_buf[1] = (uint8_t)(sensor_addr & 0xFF);
  uplink_buf[2] = (uint8_t)(payload_len >> 8);
  uplink_buf[3] = (uint8_t)(payload_len & 0xFF);

  if (payload_len > 0) {
    memcpy(&uplink_buf[4], frame->payload, payload_len);
  }

  /* Forward to WAN */
  if (mcu_wan_enqueue_uplink(HANDLER_ZIGBEE, uplink_buf,
                             (uint16_t)(4 + payload_len))) {
    g_stats.uplink_forwarded++;
    ESP_LOGD(TAG, "Forwarded Zigbee uplink from node 0x%04X (%u bytes)",
             sensor_addr, payload_len);
  } else {
    g_stats.uplink_queue_full++;
    ESP_LOGW(TAG, "WAN uplink queue full (Zigbee, node=0x%04X)", sensor_addr);
  }
}

/* ===== Statistics Logging ===== */

static void zigbee_nostack_connect_log_stats(void) {
  zigbee_nostack_connect_stats_t s;
  zigbee_nostack_connect_get_stat(&s);

  ESP_LOGI(TAG, "=== Zigbee no-stack connect statistics ===");
  ESP_LOGI(TAG, "TX_OK: %lu, RX_OK: %lu, RX_ERR: %lu", s.tx_ok, s.rx_ok,
           s.rx_error);
  ESP_LOGI(TAG, "Uplink forwarded: %lu, Uplink queue full: %lu",
           s.uplink_forwarded, s.uplink_queue_full);
  ESP_LOGI(TAG, "Downlink enqueued: %lu, Downlink dropped: %lu",
           s.downlink_enqueued, s.downlink_dropped);
  ESP_LOGI(TAG, "========================================");
}
