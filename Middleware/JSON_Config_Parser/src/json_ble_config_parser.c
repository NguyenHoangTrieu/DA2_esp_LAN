/**
 * @file json_ble_config_parser.c
 * @brief BLE-specific JSON configuration parser implementation
 */

#include "json_ble_config_parser.h"
#include "cJSON.h"
#include "esp_log.h"
#include <string.h>

static const char *TAG = "BLE_PARSER";

/* ============================================================================
 * Hardcoded BLE Function Names
 * ========================================================================== */

static const char *BLE_FUNCTION_NAMES[JSON_BLE_FUNC_MAX] = {
    // Core Functions (0-14)
    "MODULE_HW_RESET",
    "MODULE_SW_RESET",
    "MODULE_FACTORY_RESET",
    "MODULE_GET_INFO",
    "MODULE_SET_NAME",
    "MODULE_SET_COMM_CONFIG",
    "MODULE_SET_RF_PARAMS",
    "MODULE_ENTER_CMD_MODE",
    "MODULE_ENTER_DATA_MODE",
    "MODULE_START_BROADCAST",
    "MODULE_CONNECT",
    "MODULE_DISCONNECT",
    "MODULE_GET_CONNECTION_STATUS",
    "MODULE_ENTER_SLEEP",
    "MODULE_WAKEUP",
    // Promoted Optional Functions (15-19) - PC App scan/send/discover workflow
    "MODULE_START_DISCOVERY",
    "MODULE_SEND_DATA",
    "MODULE_GET_DIAGNOSTICS",
    "MODULE_DISCOVER_SERVICES",
    "MODULE_DISCOVER_CHARACTERISTICS"
};

/* ============================================================================
 * Internal Helper Functions
 * ========================================================================== */

/**
 * @brief Lookup function ID by name
 */
static json_ble_function_id_t get_function_id(const char *function_name) {
  for (int i = 0; i < JSON_BLE_FUNC_MAX; i++) {
    if (strcmp(function_name, BLE_FUNCTION_NAMES[i]) == 0) {
      return (json_ble_function_id_t)i;
    }
  }
  return JSON_BLE_FUNC_MAX; // Not found
}

/**
 * @brief Parse GPIO control array from JSON
 */
static esp_err_t parse_gpio_array(cJSON *gpio_array, gpio_control_t *gpio_out,
                                  uint8_t *count) {
  if (!cJSON_IsArray(gpio_array)) {
    *count = 0;
    return ESP_OK; // Empty array is valid
  }

  int array_size = cJSON_GetArraySize(gpio_array);
  if (array_size > MAX_GPIO_ACTIONS) {
    ESP_LOGE(TAG, "Too many GPIO actions: %d (max %d)", array_size,
             MAX_GPIO_ACTIONS);
    return ESP_ERR_INVALID_ARG;
  }

  *count = 0;
  cJSON *gpio_item = NULL;
  cJSON_ArrayForEach(gpio_item, gpio_array) {
    cJSON *pin = cJSON_GetObjectItem(gpio_item, "pin");
    cJSON *state = cJSON_GetObjectItem(gpio_item, "state");

    if (!cJSON_IsString(pin)) {
      ESP_LOGE(TAG, "Invalid GPIO pin field");
      return ESP_ERR_INVALID_ARG;
    }

    if (!cJSON_IsString(state)) {
      ESP_LOGE(TAG, "Invalid GPIO state field");
      return ESP_ERR_INVALID_ARG;
    }

    // Copy pin ID
    strncpy(gpio_out[*count].pin, pin->valuestring, MAX_PIN_ID_LEN - 1);
    gpio_out[*count].pin[MAX_PIN_ID_LEN - 1] = '\0';

    // Parse state
    if (strcmp(state->valuestring, "HIGH") == 0) {
      gpio_out[*count].state = true;
    } else if (strcmp(state->valuestring, "LOW") == 0) {
      gpio_out[*count].state = false;
    } else {
      ESP_LOGE(TAG, "Invalid GPIO state: %s", state->valuestring);
      return ESP_ERR_INVALID_ARG;
    }

    (*count)++;
  }

  return ESP_OK;
}

/**
 * @brief Parse single BLE function from JSON
 */
static esp_err_t parse_function(cJSON *func_json,
                                json_ble_function_config_t *func_out) {
  // Extract function_name
  cJSON *function_name = cJSON_GetObjectItem(func_json, "function_name");
  if (!cJSON_IsString(function_name)) {
    ESP_LOGE(TAG, "Missing or invalid 'function_name' field");
    return ESP_ERR_INVALID_ARG;
  }

  // Lookup function ID
  json_ble_function_id_t func_id = get_function_id(function_name->valuestring);
  if (func_id == JSON_BLE_FUNC_MAX) {
    ESP_LOGE(TAG, "Unknown BLE function: %s", function_name->valuestring);
    return ESP_ERR_INVALID_ARG;
  }
  func_out->function_id = func_id;
  func_out->available = true;

  // Extract command
  cJSON *command = cJSON_GetObjectItem(func_json, "command");
  if (cJSON_IsString(command)) {
    strncpy(func_out->command, command->valuestring, BLE_COMMAND_LEN - 1);
    func_out->command[BLE_COMMAND_LEN - 1] = '\0';
  } else {
    func_out->command[0] = '\0';
  }

  // Extract is_prefix
  cJSON *is_prefix = cJSON_GetObjectItem(func_json, "is_prefix");
  func_out->is_prefix = cJSON_IsBool(is_prefix) ? cJSON_IsTrue(is_prefix) : false;

  // Extract is_hex
  cJSON *is_hex = cJSON_GetObjectItem(func_json, "is_hex");
  func_out->is_hex = cJSON_IsBool(is_hex) && cJSON_IsTrue(is_hex);

  // Extract GPIO start control
  cJSON *gpio_start = cJSON_GetObjectItem(func_json, "gpio_start_control");
  esp_err_t ret = parse_gpio_array(gpio_start, func_out->gpio_start,
                                   &func_out->gpio_start_count);
  if (ret != ESP_OK) {
    return ret;
  }

  // Extract delay_start
  cJSON *delay_start = cJSON_GetObjectItem(func_json, "delay_start");
  func_out->delay_start_ms =
      cJSON_IsNumber(delay_start) ? delay_start->valueint : 0;

  // Extract expect_response
  cJSON *expect_response = cJSON_GetObjectItem(func_json, "expect_response");
  if (cJSON_IsString(expect_response)) {
    strncpy(func_out->expect_response, expect_response->valuestring,
            BLE_RESPONSE_LEN - 1);
    func_out->expect_response[BLE_RESPONSE_LEN - 1] = '\0';
  } else {
    func_out->expect_response[0] = '\0';
  }

  // Extract timeout
  cJSON *timeout = cJSON_GetObjectItem(func_json, "timeout");
  func_out->timeout_ms = cJSON_IsNumber(timeout) ? timeout->valueint : 0;

  // Extract GPIO end control
  cJSON *gpio_end = cJSON_GetObjectItem(func_json, "gpio_end_control");
  ret =
      parse_gpio_array(gpio_end, func_out->gpio_end, &func_out->gpio_end_count);
  if (ret != ESP_OK) {
    return ret;
  }

  // Extract delay_end
  cJSON *delay_end = cJSON_GetObjectItem(func_json, "delay_end");
  func_out->delay_end_ms = cJSON_IsNumber(delay_end) ? delay_end->valueint : 0;

  ESP_LOGI(TAG, "Parsed function: %s (ID=%d)", function_name->valuestring,
           func_id);

  return ESP_OK;
}

/* ============================================================================
 * Public API Implementation
 * ========================================================================== */

esp_err_t json_ble_config_parse(const char *json_str,
                                json_ble_module_config_t *config) {
  if (json_str == NULL || config == NULL) {
    ESP_LOGE(TAG, "Invalid arguments");
    return ESP_ERR_INVALID_ARG;
  }

  // Parse metadata using common parser
  esp_err_t ret = json_config_parse_metadata(json_str, &config->metadata);
  if (ret != ESP_OK) {
    ESP_LOGE(TAG, "Failed to parse metadata");
    return ret;
  }

  // Verify module type is BLE
  if (strcmp(config->metadata.module_type, "BLE") != 0) {
    ESP_LOGE(TAG, "Module type is not BLE: %s", config->metadata.module_type);
    return ESP_ERR_INVALID_ARG;
  }

  // Parse JSON again to get functions array
  cJSON *root = cJSON_Parse(json_str);
  if (root == NULL) {
    ESP_LOGE(TAG, "Failed to parse JSON");
    return ESP_ERR_INVALID_ARG;
  }

  // Extract functions array
  cJSON *functions = cJSON_GetObjectItem(root, "functions");
  if (!cJSON_IsArray(functions)) {
    ESP_LOGE(TAG, "Missing or invalid 'functions' array");
    cJSON_Delete(root);
    return ESP_ERR_INVALID_ARG;
  }

  int func_array_size = cJSON_GetArraySize(functions);
  if (func_array_size > BLE_MAX_FUNCTIONS) {
    ESP_LOGE(TAG, "Too many functions: %d (max %d)", func_array_size,
             BLE_MAX_FUNCTIONS);
    cJSON_Delete(root);
    return ESP_ERR_INVALID_ARG;
  }

  // Initialize all functions as unused
  config->function_count = 0;
  memset(config->functions, 0, sizeof(config->functions));

  // Parse each function
  cJSON *func_json = NULL;
  cJSON_ArrayForEach(func_json, functions) {
    json_ble_function_config_t temp_func;
    ret = parse_function(func_json, &temp_func);
    if (ret != ESP_OK) {
      ESP_LOGE(TAG, "Failed to parse function");
      cJSON_Delete(root);
      return ret;
    }

    // Store function indexed by function_id
    config->functions[temp_func.function_id] = temp_func;
    config->function_count++;
  }

  ESP_LOGI(TAG, "BLE config parsed successfully: %d functions",
           config->function_count);

  cJSON_Delete(root);
  return ESP_OK;
}
