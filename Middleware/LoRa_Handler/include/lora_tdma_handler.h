...
#define LORA_HANDLER_MAX_PAYLOAD                                               \
  (E32_TRANSPARENT_MAX_SIZE - LORA_HANDLER_HEADER_SIZE)
...
/* Default crypto key (XOR stream). Only first g_lora_handler_crypto_key_len
 * bytes are used.
 */
 ...
 */
void lora_handler_get_stats(lora_handler_ctx_t *ctx, lora_handler_stats_t *out);
