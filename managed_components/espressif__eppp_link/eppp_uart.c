...
static esp_err_t transmit_generic(struct eppp_uart *handle, int channel,
  ...
  ESP_LOG_BUFFER_HEXDUMP("ppp_uart_send", out_buf, len + sizeof(struct header),
                         ESP_LOG_DEBUG);
                         
  uint8_t *p = out_buf;
  size_t remaining = len + sizeof(struct header);
  while(remaining > 0) {
      size_t chunk = (remaining > 64) ? 64 : remaining;
      uart_write_bytes(handle->uart_port, p, chunk);
...
static esp_err_t transmit(void *h, void *buffer, size_t len) {
  ...
  struct eppp_uart *uart_handle =
      __containerof(handle, struct eppp_uart, parent);
  return transmit_generic(uart_handle, 0, buffer, len);
}

#ifdef CONFIG_EPPP_LINK_CHANNELS_SUPPORT
static esp_err_t transmit_channel(esp_netif_t *netif, int channel, void *buffer,
                                  size_t len) {
...
static esp_err_t init_uart(struct eppp_uart *h,
                           struct eppp_config_uart_s *config) {
  ...
  ESP_RETURN_ON_ERROR(uart_driver_install(h->uart_port, config->rx_buffer_size,
                                          0, config->queue_size,
                                          &h->uart_event_queue, 0),
                      TAG, "Failed to install UART");
  ESP_RETURN_ON_ERROR(uart_param_config(h->uart_port, &uart_config), TAG,
                      "Failed to set params");
  ESP_RETURN_ON_ERROR(uart_set_pin(h->uart_port, config->tx_io, config->rx_io,
                                   config->rts_io, config->cts_io),
                      TAG, "Failed to set UART pins");
  ESP_RETURN_ON_ERROR(uart_set_rx_timeout(h->uart_port, 1), TAG,
                      "Failed to set UART Rx timeout");
  return ESP_OK;
}
 ...
 */
static void process_packet(esp_netif_t *netif, uart_port_t uart_port,
  ...
  size_t read_size =
      (available_data < available_space) ? available_data : available_space;
  if (read_size > 0) {
    size_t len = uart_read_bytes(uart_port, in_buf + buf_end, read_size, 0);
    ESP_LOG_BUFFER_HEXDUMP("ppp_uart_recv", in_buf + buf_end, len,
                           ESP_LOG_DEBUG);
  ...
  while ((buf_end - buf_start) >= sizeof(struct header)) {
    ...
    if ((buf_end - buf_start) < total_packet_size) {
      ESP_LOGD(TAG, "Incomplete packet: got %d bytes, need %d bytes",
               (buf_end - buf_start), total_packet_size);
      break;
    }

    // Got a complete packet, pass it to network
    if (channel == 0) {
      esp_netif_receive(netif, in_buf + buf_start + sizeof(struct header),
                        payload_size, NULL);
    } else {
#ifdef CONFIG_EPPP_LINK_CHANNELS_SUPPORT
      struct eppp_handle *handle = esp_netif_get_io_driver(netif);
      ...
      if (h->parent.channel_rx) {
        h->parent.channel_rx(netif, channel,
                             in_buf + buf_start + sizeof(struct header),
                             payload_size);
      }
#endif
    }
    ...
    if (buf_start > (sizeof(in_buf) / 2) ||
        (sizeof(in_buf) - buf_end) < UART_BUF_SIZE) {
  ...
  recover:
    ...
    uint8_t *next_magic =
        memchr(in_buf + buf_start + 1, HEADER_MAGIC, buf_end - buf_start - 1);
    if (next_magic) {
...
esp_err_t eppp_perform(esp_netif_t *netif) {
  ...
  uart_event_t event = {};
  if (xQueueReceive(h->uart_event_queue, &event, pdMS_TO_TICKS(100)) !=
      pdTRUE) {
    return ESP_OK;
  }
  ...
  if (event.type == UART_DATA || event.type == UART_BUFFER_FULL || event.type == UART_FIFO_OVF) {
    if (event.type == UART_BUFFER_FULL) {
      ...
      ESP_LOGW(TAG, "Hardware FIFO Overflow - flushing UART input");
      uart_flush_input(h->uart_port);
      xQueueReset(h->uart_event_queue);
    }

#ifdef CONFIG_EPPP_LINK_USES_PPP
    /* PPP mode: Drain the entire UART ring buffer.
     * Waiting for the event queue ensures we read data in chunks (efficient for
     * LwIP), but we must drain completely to prevent the ring buffer from
     * overflowing at high baud rates (921600). */
    static uint8_t buffer[UART_BUF_SIZE] = {};
    size_t available = 0;
    do {
      uart_get_buffered_data_len(h->uart_port, &available);
      if (available == 0)
        break;

      size_t to_read = (available > UART_BUF_SIZE) ? UART_BUF_SIZE : available;
      int rd = uart_read_bytes(h->uart_port, buffer, to_read, 0);
      if (rd > 0) {
        ESP_LOGI(TAG, "PPP UART Recv: %d bytes (available: %d)", rd, available);
        ESP_LOG_BUFFER_HEXDUMP("ppp_uart_recv", buffer, rd, ESP_LOG_DEBUG);
        esp_netif_receive(netif, buffer, rd, NULL);
      }
    } while (available > 0);
#else
    /* TUN/packet mode */
...
static esp_err_t post_attach(esp_netif_t *esp_netif, void *args) {
  ...
  ESP_RETURN_ON_FALSE(h, ESP_ERR_INVALID_ARG, TAG,
                      "Transport handle cannot be null");
  ...
  ESP_RETURN_ON_ERROR(esp_netif_set_driver_config(esp_netif, &driver_ifconfig),
                      TAG, "Failed to set driver config");
  ESP_LOGI(TAG, "EPPP UART transport attached to EPPP netif %s",
